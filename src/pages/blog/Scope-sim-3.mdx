...
As stated before in other articles, I'm not too good in math, but am able to gather info & deduce. That's what I'll do here. Let's follow along.

The **equation of a parabola**, symmetric on the Y axis, can be defined as `y^2 = 4ax`, where `a` is the focal point (so, paraxial focal length) of the parabola.
...
Then, I have drawn the beam of rays coming from infinity, hitting the parabola, and reflecting back to (focal_length, 0).

![](/pics/photoscope_3/parallel_rays.png)

Then, I'll need to be able to actually trace rays coming from a non-infinity distance, and bouncing on the parabolic surface. For that, you just need to remember that a ray reflects on the other side of the normal to a curve, with the same angle to this normal.

So we first need to get tangents to points on the parabola, then, from there, normals to points on the parabola. The tangent to a point on a curve can be seen as the derivative of the curve at this point, and the normal can be seen as the tangent rotated 90°.
...
And we'll compute their reflections, that is, the ray going from the (x, y) point on the parabola, to the (x1, 0) point on the X axis. For that, I need to know the angle between the incoming ray and the normal. I don't know how to do that, but I've been able to transform the problem to something else. I know how to get angles in a right-angled triangle. So I can just "translate" my segments to have their first point at the (0, 0) coordinate, and take the X axis as a side of the triangle.
...
Then, as we have an origin point (x, y) on the parabola, and an angle from that point, we can just project it to get the X coordinate where Y = 0. I must admit that it took me a fair bit of imaginary triangles for that :
...
It is very close to the Typescript one and could probably be optimized a lot.
