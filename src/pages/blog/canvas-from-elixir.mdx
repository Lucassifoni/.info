...
I started by simply defining a render function, taking in a game struct, some viewport options, and defining a list of operations it should apply on the canvas. `clear_screen` clearly doesn't need to get the game struct, `draw_players` and `draw_ball` need it.
...
What's cool about that is that the rendering logic and calculations are done on the server based on the game state, and becomes a simple loop of draw calls on the HTML side. But there's a bit of ifs and elses and checking done and string manipulations done at runtime, and that's something we could easily avoid while keeping the ease of having a rendered list of commands that we can read.

As some of the Canvas API functions are variadic, we will use a fixed number of parameters for every of them, even if that means filling default parameters on the elixir side when it woudln't be needed.

Let's modify the `render` function to render an interpreter as the JS code to draw on our canvas.

This map goes from an operation, to a tuple containing its operation ID, whether it is a call or a property, and in case it is a call, the number of arguments it should consume :
...
We could have a special case for the 0-arity functions, but this gives the idea. The script in the HTML template is modified :
...
And `render_commands` is modified too to just output a flat list of command IDS and arguments :
...
On the rendered HTML, the interpreted becomes a switch, consuming the right number of arguments until the end of the list of commands :
