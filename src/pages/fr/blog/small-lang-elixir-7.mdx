...
title: "Un petit langage (Ovo2) dans Elixir, pt 7<br/>Fonctionnalités bizarres et état global"
description: "Le projet part en hors-piste"
pubDate: "Jul 6 2023"
...
- Partie 1 : [collecte des besoins d'une implémentation précédente](/fr/blog/small-lang-elixir)
- Partie 2 : [tokenization](/fr/blog/small-lang-elixir-2)
- Partie 3 : [parsing](/fr/blog/small-lang-elixir-3)
- Partie 4 : [émission d'AST et boucle parse-affiche-parse](/fr/blog/small-lang-elixir-4)
- Partie 5 : [évaluation](/fr/blog/small-lang-elixir-5)
- Partie 6 : [récursion basique : l'environment en tant que process](/fr/blog/small-lang-elixir-6)
- [x] Partie 7 : [fonctionnalités bizarres et état global](/fr/blog/small-lang-elixir-7)
- Partie 8 : [un environment graphique avec liveview](/fr/blog/small-lang-elixir-8)
...
<br/>
### bonks : stateful functions

I wondered what would happen if you `bonked` a lambda, meaning, what happens if you hit a function like a pinata after it has run ? The only meaningful thing I thought of would be to shed its previous result. So, I modified the tokenizer and parser to add a new syntax for that.

The `bonk` feature works with lambdas that have been declared with a `!` before their argument list.  
...
Instead of running Ovo programs by writing code and calling `Ovo.run/2` with code and some input, you can also run programs as independent `Runners` inside a stateful system. Registering a `Runner` gives back `{:ok, hash}` with an unique (collisions excepted :^)) hash, that you can keep around to call back that runner with some input. I introduced `arg/1` to the standard library, as I added `bonk/1` earlier. `arg/1` gives you the value in a positional argument.
...
I then thought that it would only be sensible to be able to `bonk` runners too, to get their previous execution result, which is popped from a stack. The programmer is responsible for not bonking a runner with an empty stack. Bonking a runner from its hash from within another ovo program is in the works.
